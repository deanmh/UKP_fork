...
st.markdown("""
    <style>
    /* Override Streamlit's primary color CSS variable */
    :root {
    ...
    (function() {{
        ...
        styleOutOptions();
        setTimeout(styleOutOptions, 100);
        setTimeout(styleOutOptions, 500);
        const observer = new MutationObserver(styleOutOptions);
...
if is_authenticated() and len(tabs) > 0:
    with tabs[0]:
        ...
        # Display buttons with proper green/red styling using Streamlit buttons with CSS and JS override
        roster_buttons_id = f"roster-buttons-{game_id}"
        st.markdown(f"""
            <style>
            /* Target Streamlit's internal button class for IN buttons (primary) */
            #{roster_buttons_id} ~ * button[data-testid="baseButton-primary"] .st-emotion-cache-726zfe,
            ...
            #{roster_buttons_id} ~ * button[data-testid="baseButton-primary"]:hover,
            #{roster_buttons_id} ~ * button[data-testid="baseButton-primary"]:hover .st-emotion-cache-726zfe {{
                background-color: #218838 !important;
                border-color: #218838 !important;
            }}
            /* Target Streamlit's internal button class for OUT buttons (secondary) */
            #{roster_buttons_id} ~ * button[data-testid="baseButton-secondary"] .st-emotion-cache-726zfe,
            ...
            #{roster_buttons_id} ~ * button[data-testid="baseButton-secondary"]:hover .st-emotion-cache-726zfe {{
            ...
            (function() {{
                ...
                const namespace = 'rosterButtonStyler_{game_id}';
                if (window[namespace + '_initialized']) return;
                window[namespace + '_initialized'] = true;
                
                function forceStyleButtons() {{
                    const marker = document.getElementById('{roster_buttons_id}');
                    if (!marker) return;
                    
                    // Find all primary buttons after marker
                    const primaryButtons = document.querySelectorAll('button[data-testid="baseButton-primary"]');
                    primaryButtons.forEach(btn => {{
                        if (marker.compareDocumentPosition(btn) & Node.DOCUMENT_POSITION_FOLLOWING) {{
                            // Force green on ALL child elements and the button itself
                            ...
                            // Also set computed style to override any CSS
                            if (window.getComputedStyle(btn).backgroundColor !== 'rgb(40, 167, 69)') {{
                                btn.style.cssText += 'background-color: #28a745 !important; border-color: #28a745 !important; color: white !important;';
                            }}
                        }}
                    }});
                    
                    // Find all secondary buttons after marker
                    const secondaryButtons = document.querySelectorAll('button[data-testid="baseButton-secondary"]');
                    secondaryButtons.forEach(btn => {{
                        if (marker.compareDocumentPosition(btn) & Node.DOCUMENT_POSITION_FOLLOWING) {{
                            // Force red border on ALL child elements and the button itself
                            const allElements = btn.querySelectorAll('*');
                            allElements.forEach(el => {{
                ...
                setTimeout(forceStyleButtons, 1000);
                
                // Use setInterval to continuously apply styles (every 200ms)
                const intervalId = setInterval(forceStyleButtons, 200);
        ...
        with st.expander("**Substitutes**", expanded=False):
            if substitutes:
                ...
                # Reuse the same CSS/JS styling from main roster
                sub_buttons_id = f"sub-buttons-{game_id}"
                st.markdown(f"""
                    <style>
                    /* Target Streamlit's internal button class for IN buttons (primary) */
                    #{sub_buttons_id} ~ * button[data-testid="baseButton-primary"] .st-emotion-cache-726zfe,
                    ...
                    #{sub_buttons_id} ~ * button[data-testid="baseButton-primary"]:hover,
                    #{sub_buttons_id} ~ * button[data-testid="baseButton-primary"]:hover .st-emotion-cache-726zfe {{
                        background-color: #218838 !important;
                        border-color: #218838 !important;
                    }}
                    /* Target Streamlit's internal button class for OUT buttons (secondary) */
                    #{sub_buttons_id} ~ * button[data-testid="baseButton-secondary"] .st-emotion-cache-726zfe,
                    ...
                    #{sub_buttons_id} ~ * button[data-testid="baseButton-secondary"]:hover .st-emotion-cache-726zfe {{
                        background-color: rgba(220, 53, 69, 0.1) !important;
                    }}
                    </style>
                    <script>
                    (function() {{
                        // Use a unique namespace to avoid conflicts
                        const namespace = 'subButtonStyler_{game_id}';
                        if (window[namespace + '_initialized']) return;
                        window[namespace + '_initialized'] = true;
                        
                        function forceStyleButtons() {{
                            const marker = document.getElementById('{sub_buttons_id}');
                            if (!marker) return;
